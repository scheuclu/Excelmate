...
function AISOLVE(input, inputColumn, outputColumn, exampleInput, exampleOutput){
  const prompt = 'Give me the output, given an input. The name of the input is ' + inputColumn + ', the name of the output is ' + outputColumn + '. An example: the input "'+exampleInput+'" should give the output "'+ exampleOutput +'". The output for the input "'+input+'" is'; 
  console.log(prompt);
  const result = AI(prompt, 0, "text-davinci-003");
  console.log(result);
  return result.replace(/^"/g, '').replace(/["|.]{0,2}$/, '');
}
